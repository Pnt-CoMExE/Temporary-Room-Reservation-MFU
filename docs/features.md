...
- **Role mapping (`resolveUserType.ts`):** Exact `@mfu.ac.th` → internal; students (`@lamduan.mfu.ac.th`) and other domains → external; admin via Admin UI promote (DB) or `DEV_ADMIN_EMAILS` for UAT bootstrap.
