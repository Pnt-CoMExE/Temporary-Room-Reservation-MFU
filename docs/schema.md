...
- `user_type` (varchar 50): Role — `admin`, `internal`, or `external`. From `resolveUserType()`: exact `@mfu.ac.th` → internal; `@lamduan.mfu.ac.th` / others → external; `admin` only via Admin UI promote (persisted in DB) or `DEV_ADMIN_EMAILS` for UAT bootstrap.
